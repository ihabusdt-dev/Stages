...
def git_backup(stage, file_path):
    try:
        ...
        subprocess.run(["git", "add", backup_file], stderr=subprocess.DEVNULL)
        subprocess.run(["git", "commit", "-m", f"backup {stage}"], stderr=subprocess.DEVNULL)
        subprocess.run(["git", "push", "origin", "main", "--force"], stderr=subprocess.DEVNULL)
...
def ffmpeg_speed(inp, out):
    cmd = [FFMPEG_BIN, "-y", "-loglevel", "quiet", "-i", inp]
    # 🔥 تكرار الفلتر
    for _ in range(FILTER_REPEAT):
        cmd += ["-filter_script:a", FILTER_FILE]
...
def main():
    ...
    while True:
        ...
        if stage % BACKUP_EVERY == 0:
            git_backup(stage, current_input)
